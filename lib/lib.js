...
function getChordMatches(inputChords) {
    ...
    chordData.forEach(row => {
        let key = row[0]; // Key name (e.g., "C Locrian")
        ...
        inputChords.forEach(inputChord => {
            ...
            if (matchIndex !== -1) {
                ...
                // Assign Roman numeral placement

                let placement = romanNumerals[matchIndex];

                // Adjust major/minor (capital for major, lowercase for minor)
                if (placement) {
                    let isMinor = /m|dim|°/.test(inputChord);
                    placement = isMinor ? placement.toLowerCase() : placement;
                }
                
                chordPlacements.push({ chord: inputChord, placement });
            } else {
                // Unmatched chords return empty placement
                chordPlacements.push({ chord: inputChord, placement: "" });
            }
        });

        // 🔹 Generate suggested chords (2-3 unused chords from the key)
        let usedChords = chordPlacements.map(p => p.chord);
        let suggestions = chords
            .map((chord, idx) => ({ chord, placement: romanNumerals[idx] })) // Add placements
            .filter(p => !usedChords.includes(p.chord) && p.placement !== "I" && !p.chord.includes("dim")) // Avoid root & dim
        ...
        if (score > 0) {
            scoredMatches.push({ key, score, chordPlacements, suggestions });
        }
    });
